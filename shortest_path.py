...
def generate_test_input_networkx_graph(rand, theta):
    """Reads in graph from input file and generates.
    ...
    ge_target_graphs.append(target_graph)
    ge_graphs.append(ge_graph)

    return ge_input_graphs, ge_target_graphs, ge_graphs
...
def create_placeholders(rand, batch_size, num_nodes_min_max, theta):
  ...
  """
    ...
    input_graphs_ge, target_graphs_ge, _ = generate_test_input_networkx_graph(rand, theta)
    # Create some example data for inspecting the vector sizes.
    input_graphs, target_graphs, _ = generate_networkx_graphs(
        rand, batch_size, num_nodes_min_max, theta)

    input_tr_ph = utils_tf.placeholders_from_networkxs(input_graphs)

    input_ge_ph = utils_tf.placeholders_from_networkxs(input_graphs_ge)
    target_tr_ph = utils_tf.placeholders_from_networkxs(target_graphs)
    target_ge_ph = utils_tf.placeholders_from_networkxs(target_graphs_ge)
    return input_tr_ph, target_tr_ph, input_ge_ph, target_ge_ph
...
def create_feed_dict_test_graph(rand, theta, input_ph,
                     target_ph):
    """Creates placeholders for the model training and evaluation.
  ...
  Returns:
    ...
    raw_graphs: The `dict` of raw networkx graphs.
  """
    inputs, targets, raw_graphs = generate_test_input_networkx_graph(
        rand, theta)
    input_graphs = utils_np.networkxs_to_graphs_tuple(inputs)
    target_graphs = utils_np.networkxs_to_graphs_tuple(targets)
    feed_dict = {input_ph: input_graphs, target_ph: target_graphs}
...
input_tr_ph, target_tr_ph, input_ge_ph, target_ge_ph = create_placeholders(rand, batch_size_tr,
...
output_ops_tr = model(input_tr_ph, num_processing_steps_tr)
output_ops_ge = model(input_ge_ph, num_processing_steps_ge)

# Training loss.
loss_ops_tr = create_loss_ops(target_tr_ph, output_ops_tr)
# Loss across processing steps.
loss_op_tr = sum(loss_ops_tr) / num_processing_steps_tr
# Test/generalization loss.
loss_ops_ge = create_loss_ops(target_ge_ph, output_ops_ge)
...
# Lets an iterable of TF graphs be output from a session as NP graphs.
input_ph_tr, target_ph_tr = make_all_runnable_in_session(input_tr_ph, target_tr_ph)
input_ph_ge, target_ph_ge = make_all_runnable_in_session(input_ge_ph, target_ge_ph)
...
for iteration in range(last_iteration, num_training_iterations):
    ...
    feed_dict, _ = create_feed_dict(rand, batch_size_tr, num_nodes_min_max_tr,
                                    theta, input_ph_tr, target_ph_tr)
    ...
    if elapsed_since_last_log > log_every_seconds:
        ...
        feed_dict, raw_graphs = create_feed_dict_test_graph(
            rand, theta, input_ph_ge, target_ph_ge)
